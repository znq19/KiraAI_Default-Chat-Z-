...
- **并行媒体识别**：图片 VLM 与语音 STT 并行预处理，推送时零等待，三级并发控制可配（v1.8.9 修复调度竞态后**真正生效**，见更新日志）
...
- **修复：预取作用域写宽 —— "没进批次"的围观图也被预取（白烧 VLM）**
  - **现象**（用户日志）：群里没人 @ 它的图片/表情，到达瞬间就 `预取启动 1 项` +
    ...
    识别成本全白付。
  - **根因**：v1.8.4 起的调用点把"**已进缓冲**"当成了"**会被送进 LLM**"——对**前文阶段**
    （唤醒之前、只作上下文、窗口满会被弹掉）的消息也打了预取标记；该调用点此前被钩子顺序
    竞态打死（预取从未真正跑过），竞态修复后它"活"了 → 作用域问题第一次显形。
    （z 版默认"接收非唤醒消息"=关、且默认"仅唤醒识别"=开，所以**默认配置下不会遇到**；
    显式打开「接收非唤醒消息」+关闭「仅唤醒识别」的组合会命中，S 版默认配置则直接命中。）
  - **修复（两步）**：
  ...
  - **日志文案改准确**：`预取启动 N 项（sid，进批次即识别 / 起批预热前文，…）`。
  - **回归测试** `tests/test_prefetch_scope.py`（5 场景，与 s 版同源）：
    P1 前文图不预取（**在修复前的代码上 FAIL = 反向验证**）、P2 起批预热、P3 批次内非唤醒图仍预取、
    P4 主动命中补标记、P5 对照（仅唤醒识别语义不变）。
- 版本 v1.8.9 → v1.8.10（与 s 版 v2.5.19 同步；`media_recognize.py` 两版逐字节一致）
...
- **修复「真·媒体预处理（预取）」实际未生效 —— 识别一直要等批次被推送**（用户反馈："批次内收到图没有秒启动识别，而是在等开始推批次"）
  ...
  - **回归测试** `tests/test_prefetch_timing.py`（7 场景，含"旧顺序 0 次 VLM"的反向验证）。**在 v1.8.8 上 A/E 失败**（预取 0 次 VLM），本版全过。
- **修复「子代理（SubAgent）跑任务时主聊天的统计被带偏」**：子代理插件用**携带真实会话 sid 的桩事件**派发 `ON_LLM_RESPONSE`（为了让 file 插件等按会话作用域的管控对子代理与主 AI 一致），本插件会把它当成主 bot 的最终回复——**存在感时间线 +1、扣分**。
  - **修复**：`ON_LLM_REQUEST` 登记"框架真实批次"（这个钩子只有框架自己会派发，任何自造桩事件都不会经过），`on_llm_response_enhance` 只处理登记过的事件；若桩事件带显式标记（`extra["_subagent_stub"]` / `is_subagent_stub`，子代理插件 v1.1.3 起提供）也直接跳过。真实批次 100% 经过 `ON_LLM_REQUEST` ⇒ **主流程零影响**（判定异常时按"真实批次"放行，宁可照旧工作也不误伤）。
  - **回归测试** `tests/test_foreign_event.py`（G1-G3）：**在 v1.8.8 上 G1/G3 失败**（存在感被污染），本版通过。
...
- 版本 v1.8.8 → v1.8.9（与 s 版 v2.5.18 同步；`queue_merge.py` / `media_recognize.py` 两版逐字节一致）
...
- **真·预处理（本次新增）**：消息**确定进入批次**（`event.buffer()`）时立刻在后台预取它的媒体 —— 而这段时间正是「上一个批次的 LLM 还在跑 / 本批次在队列里排队」的空窗。放行时 stage2 直接从结果池命中，**本批次关键路径零识别开销**（实测放行后 VLM=0）。被 `discard()` 的消息走不到 buffer ⇒ 不预取、不浪费。私聊同样生效。
  > ⚠ 更正（v1.8.9）：该调度点在 `handle_msg`（先注册的钩子）里，会被 stage1 的钩子顺序竞态打死（worker 抢跑时媒体表还是空的，读到空即退出且不重试）——**本条直到 v1.8.9 才真正生效**，详见 v1.8.9 更新日志。
